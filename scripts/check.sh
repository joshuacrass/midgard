...
cd "$ROOT"
while IFS= read -r -d '' file; do bash -n "$file"; done < <(find scripts config tests -name '*.sh' -print0)
bash -n bootstrap.sh
...
shellcheck -e SC1091 bootstrap.sh scripts/*.sh tests/*.sh config/versions.sh
